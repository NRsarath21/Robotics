...
//For Left Sensor A3
if (Left(A3) <0.17) // If left sensor detected then turn Right
{
...
if(Middle(A4) < 0.10) // If A4 voltage greater than 3.5
{
digitalWrite(MiddleLed,LOW); // off if white
  ...
  //For Right Sensor A5
if(Right(A5) <0.18) // If Right sensor detected then turn Right
{
...
if((Left(A3) > 0.17 && Right(A5) > 0.18)) //If both are white run forward
{
    forward();
}
}
...
void forward()
{
  servoLeft.writeMicroseconds(1650); // Full speed forward 1700
